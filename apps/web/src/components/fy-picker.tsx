...
export function FyPicker({ fy, available }: { fy: FiscalYear; available: FiscalYear[] }) {
  ...
  return (
    <label className="flex items-center gap-2">
      <span className="eyebrow">{strings.common.fiscalYear}</span>
      <select
        value={fy}
        onChange={(event) => onChange(event.target.value)}
        aria-label={strings.common.fiscalYear}
        className="figure cursor-pointer border border-[color:var(--color-rule-strong)] bg-[color:var(--color-paper)] px-2 py-1 text-[13px] text-[color:var(--color-ink)]"
      >
        {available.map((year) => (
